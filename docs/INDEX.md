...
| 2026-05-16 | ready to dispatch | [`plans/2026-05-16-remote-control-R5e-entry-enrichment.md`](plans/2026-05-16-remote-control-R5e-entry-enrichment.md) — R-5e: server-side enrichment of `EntrySummary` (full markdown + image content blocks + tool-call + plan summaries) + `agent_session_message_appended` payload (entry_index + role + preview). Additive — old clients unaffected. New `remote.solution_agent.get_session_entry` tool. Spk-editor side only; R-5e-client update follows in a separate phase. |
