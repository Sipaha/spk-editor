...
| 2026-05-17 | ready to dispatch | [`plans/2026-05-17-remote-control-R6e-pagination-incremental-resume.md`](plans/2026-05-17-remote-control-R6e-pagination-incremental-resume.md) — R-6e: cursor-style pagination on `get_session` (new `before_index` / `after_index` / `count` params + `total_count` + always-populated `EntrySummary.index`). `list_sessions` pagination too (defensive). Client uses count=50 initial fetch + "Load older" affordance + `after_index=lastSeenEntryIndex` on reconnect instead of full re-fetch. Two sub-agents: server first, client second. |
