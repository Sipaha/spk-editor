...
|---|---|---|---|
| **R-5a** Android client bootstrap | HEAVY | `spk-editor-android-client` (sibling of `spk-editor`) | **Shipped 2026-05-16**: sibling repo commit `77eb966`; two-module Gradle (`:core` JVM + `:app` Compose stub), 30 green `:core` unit tests, `:app:assembleDebug` confirmed to fail only on missing `ANDROID_HOME`. See [`plans/2026-05-16-remote-control-R5a-android-bootstrap.md`](../plans/2026-05-16-remote-control-R5a-android-bootstrap.md). |
| **R-5a-followup** :cli + live integration test | LIGHT | (same sibling repo) | **Shipped 2026-05-16**: sibling-repo commit `4e478f1`. `:cli` JVM smoke client + six-step `LiveEditorIntegrationTest` (still tag-gated as `integration`, default `:core:test` keeps 30 PASSED baseline). Supervisor-verified including the `-DincludeTags=integration` discovery path. Sub-agent flagged two deferred follow-ups: `:core` exposes `OkHttpClient.Builder` via constructor default arg (API-surface leak; fix by hiding the type in a future refactor pass), and a pre-existing `@ExperimentalCoroutinesApi` warning on `RemoteClient.kt:86`. |
| **R-5b** QR scanner | HEAVY | (same sibling repo) | **Planned, blocked on Android SDK install.** Plan-doc at [`plans/2026-05-16-remote-control-R5b-qr-scanner.md`](../plans/2026-05-16-remote-control-R5b-qr-scanner.md). zxing-android-embedded; replaces R-5a's paste-URL stub. |
| **R-5c** Solutions/sessions list UI | HEAVY | (same sibling repo) | **Planned, blocked on R-5b + SDK.** Plan-doc at [`plans/2026-05-16-remote-control-R5c-solutions-sessions-list.md`](../plans/2026-05-16-remote-control-R5c-solutions-sessions-list.md). Navigation Compose, live-subscribe to `agent_session_state_changed`. |
| **R-5d** Chat with streaming + cancel-turn | HEAVY | (same sibling repo) | **Planned, blocked on R-5c + SDK.** Plan-doc at [`plans/2026-05-16-remote-control-R5d-chat-streaming.md`](../plans/2026-05-16-remote-control-R5d-chat-streaming.md). Closes the R-5 arc. |
| **R-6** Android client polish | HEAVY | (same sibling repo) | Depends on R-5 closing. FCM/push, reconnect, multi-server. No plan-doc yet. |
