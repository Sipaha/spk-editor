...
- IDEA branches popup action header (decision 24): only **Push** is wired solution-wide (`git_ui::providers` push provider → `open_solution_push_dialog`), because that's the only solution-wide op exposed as a dispatchable UI entry. **Update Project** dispatches `git::Fetch` and **Commit** opens the git panel — both **active-repo, not solution-wide** — because `batch_fetch`/`batch_pull` exist only as `solution.git.*` MCP tools (no UI action) and there is no solution-commit UI entry reachable from `git_ui`. The approved spec wanted all three solution-wide; making Update/Commit solution-wide needs NEW UI flows (out of scope for the action-row task). Revisit when those flows exist. **Checkout Tag or Revision…** shares the `git::Branch` dispatch with **New Branch** (the `BranchList` picker handles both via its Tags section / SHA entry) — no dedicated checkout-revision action exists.
