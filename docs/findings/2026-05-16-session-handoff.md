...
| **R-5e-client** Android client consumes enriched entries | HEAVY | `spk-editor-android-client` sibling repo | **New follow-up surfaced by R-5e**: extend `:core` DTOs (markdown/images/tool_call/plan optionals on EntrySummary), pass `include_full_content: true, include_images: true` from MainViewModel.openSession, render rich markdown + inline images + expandable tool-call rows. No plan-doc yet. |
