...
| **R-5e** Server-side enrichment for remote chat | HEAVY | spk-editor (`crates/solution_agent/src/mcp.rs` + `event_sources.rs`) | **New follow-up surfaced by R-5d**: enrich `EntrySummary` (full markdown + images + tool-call args/results) and `agent_session_message_appended` payload (`{session_id, entry_index, entry}` instead of id-only) so the phone client can render rich messages and stream without full re-fetches. No plan-doc yet. |
| **R-6** Android client polish | HEAVY | (same sibling repo) | Depends on R-5 closing (now closed). FCM/push, reconnect, multi-server. No plan-doc yet. |
