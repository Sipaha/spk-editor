...
| `crates/title_bar/src/title_bar.rs` | Embeds `solutions_ui::SolutionTabStrip` after the hamburger; project-info segment (solution name + worktree + branch) removed; uses fork-local `fork_title_bar_content_height()` for the content row. Also renders the run-config widget (`Workspace::run_config_strip`) right-aligned in the header, at the start of the right-side controls cluster (IDEA-style), and the IDEA branch widget (`render_branch_widget` → anchored `PopoverMenu<BranchesPopup>`) just left of it; registers + toggles the `git::BranchesPopupOpen` shortcut (decision 24). `render_restricted_mode` call site disabled (function kept under `#[allow(dead_code)]`) — see decision 13. | `solutions_ui` / `solutions` / `run_config_ui` / `git_ui` |
...
How to apply:
- `TitleBar` holds `branch_popover_handle: PopoverMenuHandle<BranchesPopup>` and renders `render_branch_widget` (right region, left of the run-config widget). The widget reads the active repo's branch; **detached HEAD shows `CommitDetails::short_sha()`** (don't hide the widget when `branch` is `None` but a repo exists). The `.menu()` closure builds `BranchesPopup::build(...)`.
- Keyboard `git::BranchesPopupOpen` is registered in **`title_bar`** (which legally depends on `git_ui`; the reverse would be a dependency cycle), downcasts `workspace.titlebar_item()` to `TitleBar`, and toggles the handle. `git_ui` no longer registers the action.
