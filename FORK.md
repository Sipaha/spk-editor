...
| `crates/run_config_ui` | UI for Run Configurations: the full-width toolbar strip under the title bar (config dropdown + Run / Debug / Stop), the Edit Configurations modal (schema-driven provider forms, before-launch = Save All, executors, project/global storage), the status-bar run indicator, and the per-`Workspace` `RunController` (launches via `Workspace::spawn_in_terminal` / `start_debug_session`, tracks active runs, publishes the running set + a command sink for MCP). | Touches upstream `workspace`, `zed/main.rs`, `zed/app_menus.rs`, `settings_content`, keymaps. |
