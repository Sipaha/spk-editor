...
| `crates/acp_thread/src/acp_thread.rs` | Adds `ToolCall::status_started_at: Option<chrono::DateTime<Utc>>` — stamped on the first transition into `InProgress` (both `from_acp` and `update_fields` paths), preserved across the transition to a terminal status. Lets fork-owned renderers (`solution_agent::conversation_render`, the MCP wire) display a live "ran for Xs" badge without inventing a parallel start-time table. | `solution_agent` |
