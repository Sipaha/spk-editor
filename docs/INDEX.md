...
| 2026-05-17 | ready to dispatch | [`plans/2026-05-17-remote-control-R6c-multi-server.md`](plans/2026-05-17-remote-control-R6c-multi-server.md) — R-6c-multi: PairingRepository moves from one-URL to keyed map (PairedServer with id/label/fingerprint/timestamps). New Servers list screen as nav entry when 2+ servers paired. R-6b → R-6c one-shot migration. Per-server scoping of drafts/queue/lastSeen/nav (server-id-prefixed keys). FCM push notifications (the other half of original R-6c) deferred to R-6c-push. |
