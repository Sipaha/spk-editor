...
| **R-5a-followup** :cli + live integration test | LIGHT | (same sibling repo) | In progress as of session end (background sub-agent). Adds a `:cli` JVM smoke consumer + expanded `LiveEditorIntegrationTest`. Rides on the R-5a plan-doc (no separate plan needed). |
