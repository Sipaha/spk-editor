...
| 2026-05-17 | ready to dispatch | [`plans/2026-05-17-F-subagent-indication.md`](plans/2026-05-17-F-subagent-indication.md) — F: sub-agent indication UI. Sub-agents modelled as first-class spk-editor sessions with `parent_session_id`. Server (spk-editor): `create_session` accepts parent ref, `SessionSummary` gains `total_tokens` + `parent_session_id`, new `get_session_children` tool, allow-list. Desktop: bubble strip above status row in `session_view.rs` (DFS-ordered, indent per nesting, hidden when no children). Phone: `AssistChip` LazyRow above compose. Trade-off explicit: Claude Code's internal Task tool dispatches won't surface here — parent agents must call `create_session({parent_session_id})` to be visible. |
