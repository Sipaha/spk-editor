...
| `crates/remote_control` | Headless model + JSON persistence for the (future) Remote Control surface: `RemoteControlSettings { server_address, server_port, enabled, clients }` persisted to `~/.config/spk-editor/remote-control.json` (live-watched + atomic writes via the standard `settings::watch_config_file` pattern). Generates 32-byte client secrets via `OsRng` + base64. R-1 ships state-model only — no listener / no QR / no encryption. | Emits `RemoteControlStoreEvent::Changed` on every mutation. `set_enabled(true)` just flips the bit; the listener is R-2. |
