...
| `crates/settings_content/src/settings_content.rs` | Adds `CommitViewSettingsContent` (avatars, lazy threshold, mention parsing) + nested field on `GitPanelSettingsContent`. Also adds `SolutionAgentSettingsContent { ephemeral }` (S-AI-MSG ephemeral-pool sizing). | `git_ui` (S-DET) / `solution_agent` (S-AI-MSG) |
| `crates/settings/src/vscode_import.rs` | Add `solution_agent: None` field initializer to keep VS Code import in lockstep with the new `SettingsContent.solution_agent` field. | `solution_agent` (S-AI-MSG) |
