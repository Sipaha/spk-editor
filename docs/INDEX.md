...
| 2026-05-17 | ready to dispatch | [`plans/2026-05-17-remote-control-R6f-ws-compression.md`](plans/2026-05-17-remote-control-R6f-ws-compression.md) — R-6f: enable `permessage-deflate` extension on the WebSocket listener via tokio-tungstenite's `deflate` feature. Server-side change with backwards-compat handshake (clients that don't offer compression keep working uncompressed). OkHttp limitation documented separately — Android client doesn't currently advertise the extension, so the wire stays uncompressed for our app; the server-side enablement benefits future / tungstenite-equivalent clients. |
